...
void UpperMonitorReceiveData(u8 *array)
	{
	 ...
	 {
	   parameter[PITCH_FIX] = CCW_ENCODER; //pitch轴安装方向 左边
	 }
	 else if(array[0] == 0x7B)
	 {
	   parameter[PITCH_FIX] = CW_ENCODER; //右边
	 }
	 else if(array[0] == 0x7C)
	 {
	   parameter[YAW_FIX] = CCW_ENCODER; //yaw轴安装方向 反向
	 }
	 else if(array[0] == 0x7D)
	 {
	   parameter[YAW_FIX] = CW_ENCODER; //正向
	 }
