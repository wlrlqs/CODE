...
typedef struct{
	...
	uint16_t nowAddress;
    uint8_t distinguishFlag;        //机器人ID开启识别标志位
...
void masterSlaveSend(CAN_TypeDef *CANx, uint32_t ID_CAN, masterSlaveSendStruct_t *CanData);
void masterSlaveDataUpdate(CanRxMsg *canRxMsg, masterSlaveSendStruct_t *canData);
void user_senddata(CAN_TypeDef *CANx, uint32_t ID_CAN, canUserData_t *userCanData);
