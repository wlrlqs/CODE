...
void sendPowerDataInit(void){
	...
	//  串口初始化                串口      ，   接收pin角      ，   发送pin角        ，  //中断抢占优先级            , 中断响应优先级
	supervisorData.taskEvent[WIRELESS_TASK] = xTaskCreate(powerDataTransportTask,"DATATRANS",WIRELESS_STACK_SIZE,NULL,WIRELESS_PRIORITY,&wirelessData.xHandleTask);
                          ...
                          //无线通信（统一格式）
void powerLinkInit(void){            //io口初始化
...
void GetAdcAverage(uint8_t channel,uint8_t times,f32_t *volPin){
	...
	uint8_t i;
		...
		vol =(f32_t) sum/10.0f;
		*volPin =(f32_t)((vol* 3.3f) / 4095.0f);	
		powerData.volPIn = *volPin;
		vol = 0.0f;
		sum = 0.0f;
}

void powerDataReceive(uint8_t *array){     //接收功率数据
	uint8_t cnt = 0;
...
//	BSP_UART7_DMA_SendData(Array_UART7_TX,index_ptr);
	BSP_USART_SendData(&POWER,Array_UART7_TX,index_ptr);   //向  uart7 发送数据  
...
static bool autoCharge,manualCharge;
	if(ROBOT == INFANTRY_ID || ROBOT == P_TANK_ID ||ROBOT == F_TANK_ID){
		...
		if(!getinfantryAutoData()->rotateFlag && !getinfantryAutoData()->aviodFlag){
			switch(powerData.step){
				case 0:
                    ...
                    break;
				case 1: {
                    if((powerData.loops - startTime) > 100){
                        if(judgeData.extPowerHeatData.chassis_power < 55){
                            //自动充电
                            autoCharge = true;
                            powerData.step +=1;
		...
		if(!manualCharge && !autoCharge){
            //低电压需要强制切换
			if(powerData.capVol < 17.0f)							
				getinfantryAutoData()->forceSwitch.vol_limit = true;     
            //强制切换成直连底盘
			else if(powerData.capVol > 18.0f)
			  getinfantryAutoData()->forceSwitch.vol_limit = false;
			//没有强制切换成直连底盘
			if(!getinfantryAutoData()->forceSwitch.vol_limit){    
                //shift 按下切换成链接超级电容
				if(openCap || (RC_ROTATE < -100)){		   
...
//				adcInit_cap();      
		...
		if(ROBOT == P_TANK_ID || ROBOT == F_TANK_ID){
			...
			powerData.capVol = (f32_t)powerData.volCap*20400.0f/2400.0f;
			if(powerData.capVol < 1e-4f || powerData.capVol > 1e+4f){
                //切换成直连底盘
				LINK_CHASSIS = CHASSIS_ON;    
				LINK_CAP = CAP_OFF;
