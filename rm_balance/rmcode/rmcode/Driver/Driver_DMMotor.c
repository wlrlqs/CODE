			...
			if(sum!=rs485Data.yawReceiveData[7]){
				...
				if((rs485Data.yawReceiveData[5] | (rs485Data.yawReceiveData[6]<<8)) > 4096){
					...
					switch(rs485Data.yawReceiveData[2]){
						case YAW :
							gimbal_readSlaveData(CODEBOARD_VALUE, \
                                                    (4096-((rs485Data.yawReceiveData[6]<<8) | rs485Data.yawReceiveData[5]))*2,&yawMotorData); 
							digitalIncreasing(&getGimbalData()->gimbalError[0].errorCount.u32_temp);
						break;
						case PITCH :
							gimbal_readSlaveData(CODEBOARD_VALUE, \
                                                    (4096-((rs485Data.yawReceiveData[6]<<8) | rs485Data.yawReceiveData[5]))*2,&pitchMotorData);
							digitalIncreasing(&getGimbalData()->gimbalError[1].errorCount.u32_temp);
						break;
					}
				}
		  }
