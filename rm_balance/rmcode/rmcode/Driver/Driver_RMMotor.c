...
/*******************************外部使用的变量*********************************/
motorCanDataRecv_t	 auxDriverWheelData[2];	
...
/****************************机器人类型电机变量*********************************/
motorCanDataRecv_t  swipeCardData;
motorCanDataRecv_t  dragMechanismData[2];
/*****************************************************************************/                 
...
void driver_can1_init(CAN_TypeDef* rm_canx,BSP_GPIOSource_TypeDef *rm_canx_rx,BSP_GPIOSource_TypeDef *rm_canx_tx,uint8_t Preemption,uint8_t Sub){
	...
	BSP_CAN_Mode_Init(&can1,CAN_SJW_1tq,CAN_BS2_4tq,CAN_BS1_9tq,3,CAN_Mode_Normal,Preemption,Sub);
} 
/*
***************************************************
函数名：driver_can1_init
...
void driver_can2_init(CAN_TypeDef* rm_canx,BSP_GPIOSource_TypeDef *rm_canx_rx,BSP_GPIOSource_TypeDef *rm_canx_tx,uint8_t Preemption,uint8_t Sub){
	...
	BSP_CAN_Mode_Init(&can2,CAN_SJW_1tq,CAN_BS2_4tq,CAN_BS1_9tq,3,CAN_Mode_Normal,Preemption,Sub);
}
/*
***************************************************
...
void rmmotor_senddata(CAN_TypeDef *CANx, uint32_t ID_CAN, motorSerialNumber_t *rmmotorCanData){
	uint8_t mbox;
	uint16_t i=0;
...
float V_Rotate,All_Speed=0;   
	for(uint8_t i = 0; i < NUMBER_OF_WHEEL; i++)
  	All_Speed += Wheel_Speed[i]/(REAL_MOTOR_SPEED_SCALE* (60.0f * GEAR_RATIO));
	
  V_Rotate = All_Speed/4;
	return V_Rotate;
}
/*
***************************************************
...
void gimbal_readData(CanRxMsg *can_rx_msg,rmmotorCanDataRecv_t *gimbal_data){                                       
	switch(gimbal_data -> motorID){
		...
		case MOTOR_RLX8:{
			gimbal_data->RMD_L_X8Data.molotovDataSheet.motorEncoder = ((uint16_t)can_rx_msg->Data[7] << 8) | can_rx_msg->Data[6];
			gimbal_data->RMD_L_X8Data.molotovDataSheet.motorSpeed   = ((int16_t)can_rx_msg->Data[5] << 8) | can_rx_msg->Data[4];
			gimbal_data->RMD_L_X8Data.molotovDataSheet.temperature  = (int8_t)can_rx_msg->Data[1];
			gimbal_data->RMD_L_X8Data.molotovDataSheet.iq			  = ((int16_t)can_rx_msg->Data[3] << 8) | can_rx_msg->Data[2];
		}break;
	  default: break;
	}
...
vs16 gimbal_chooseData(uint8_t dataNumber,rmmotorCanDataRecv_t *gimbal_data){                                       
	...
	switch(gimbal_data -> motorID){
		...
		case MOTOR_RLX8:{
			switch(dataNumber){
				case CODEBOARD_VALUE: dataChoose = gimbal_data->RMD_L_X8Data.molotovDataSheet.motorEncoder;break;
				case TORQUE_CURRENT: dataChoose = gimbal_data->RMD_L_X8Data.molotovDataSheet.iq;break;	
			}
		}break;
	  default: break;
	}
